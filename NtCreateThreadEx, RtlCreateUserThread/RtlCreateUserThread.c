...
int main(int argc, char *argv[]) {                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    
	...
	if (hProcess) {
		...
		pFunc = GetProcAddress(GetModuleHandleA("ntdll.dll"), "RtlCreateUserThread");
		if (pFunc == NULL) {
			puts("failed GetProcAddress Function");
			return -1;
		}
		((PFRtlCreateUserThread)pFunc)(hProcess, NULL, FALSE, 0, 0, 0, (PTHREAD_START_ROUTINE)libAddr, (PVOID)pRemoteAddr, &hThread, &cid);
		if (hThread == NULL) {
			puts("failed RtlCreateUserThread Function");
			return -1;
		}
